        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {pillars.map((pillar) => {
            ...
            return (
              <Card 
                key={pillar.title} 
                className={`${colors.bg} ${colors.border} border transition-all duration-300 relative overflow-hidden`}
              >
                {!pillar.available && (
                  <Badge 
                    className="absolute top-4 right-4 bg-muted-foreground/20 text-muted-foreground text-xs"
                  >
                    Próximamente
                  </Badge>
                )}
