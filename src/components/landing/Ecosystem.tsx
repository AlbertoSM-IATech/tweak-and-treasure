          ...
          {pillars.map((pillar, index) => {
            ...
            return (
              <AnimatedSection key={pillar.title} delay={index * 0.1}>
                <Card className={`${colors.bg} ${colors.border} border transition-all duration-300 relative overflow-hidden h-full`}>
                  {!pillar.available && (
                    <Badge className="absolute top-4 right-4 bg-muted-foreground/20 text-muted-foreground text-xs">
                      Próximamente
                    </Badge>
                  )}
